let URL_API =
  "https://api.themoviedb.org/3/list/7066537?api_key=ef1cb8e5e1dc0db221a9419c6295ed99&language=en-US";
...
function mapCards(movies) {
  const html = movies
    .map((movie) => {
      ...
      return `
      <div class="card" >
        <div class="frontWeb" style="background-image: url(//image.tmdb.org/t/p/original${
          movie.poster_path
        });"> 
          <p>${title}</p>
        </div>
        ...
        <div class="back">
          <div>
            <div class="release_date">${title} <span>(${getYearFromDate(
        movie.release_date
      )})</span></div>
            ...
            <a target="_blank" href="https://www.themoviedb.org/${isMovieOrTv}/${
        movie.id
    ...
    .join("");
  card.innerHTML = `<h1 class="heading">Movies</h1>`;
  card.innerHTML += html;
}
